...
const DropDownForm: FC<TDropDownFormProps> = ({ boardId, list, listId, setIsFormOpen }) => {
      ...
      if (list) {
        ...
        dispatch(
          addTask({
            boardId,
            listId,
            task: { taskId: v4(), taskName: text, taskDescription: '', taskOwner: 'User' },
          })
        );
        dispatch(
          addLog({
  ...
  return (
    <div className={list ? listForm : taskForm}>
      <textarea className={input} value={text} onChange={handleTextChange} autoFocus placeholder={formPlaceholder} onBlur={() => setIsFormOpen} />
